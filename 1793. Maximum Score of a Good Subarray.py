class Solution:
    def maximumScore(self, nums: List[int], k: int) -> int:
        # think of a greedy solution expanding around k, always expand to the greater value
        # another approach to populate left min index and right min index for each index
        n = len(nums)
        left = k
        right = k
        ans = nums[k]
